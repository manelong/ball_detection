...
def load_csv(csv_path, fids, frame_dir=None):
    ...
    for i in range(len(data)):
        ...
        xyvs[i] = {'center': Center(x=x, y=y, is_visible=visi), 
...
def get_clips(cfg, train_or_test='test', gt=True):
    ...
    for match in matches:
        ...
        for clip_name in clip_names:
            ...
            fids.sort()
            ball_xyvs = load_csv(clip_csv_path, fids)
            frame_names = ['{}{}'.format(fid, ext) for fid in fids]
            clip_dict[(match, clip_name)] = {'clip_dir_or_path': clip_frame_dir, 'clip_gt_dict': ball_xyvs, 'frame_names': frame_names}
    return clip_dict
            ...
            for clip_name in clip_names:
                ...
                for frame_name in os.listdir(clip_frame_dir):
                    if frame_name.endswith(self._ext):
                        fids.append(int(osp.splitext(frame_name)[0]))
                fids.sort()
                ball_xyvs = load_csv(clip_csv_path, fids, frame_dir=clip_frame_dir)
