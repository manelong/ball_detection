...
from .dataset_loader import ImageDataset, read_image, get_transform, ImageMultiballDataset
from .samplers import select_sampler, RandomSampler, select_sampler_multiball
...
def build_dataloader_multiball(cfg: DictConfig):
    dataset = select_dataset(cfg)
    ...
    train_sampler, test_sampler = select_sampler_multiball(cfg['dataloader']['sampler'], dataset)
    ...
    if model_name in ['tracknetv2', 'hrnet', 'monotrack', 'restracknetv2', 'deepball', 'ballseg']:
        train_dataset = ImageMultiballDataset(cfg,
                                     dataset=dataset.train,
                                     input_wh=input_wh,
                                     output_wh=output_wh,
                                     transform=transform_train,
        ...
        test_dataset = ImageMultiballDataset(cfg,
                                    dataset=dataset.test,
                                    input_wh=input_wh,
                                    output_wh=output_wh,
                                    transform=transform_test,
