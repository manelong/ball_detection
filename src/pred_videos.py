...
def main(
    ...
    for video_name in video_list:
        ...
        # sort_tracker = Sort(max_age=5, min_hits=3, dist_threshold=50)


        #### 多球处理 轨迹分段画图
        # for cnt, (img_path, v) in enumerate(result_dict.items()):
        #     vis_frame_path = os.path.join(vis_frame_dir, os.path.basename(img_path) + '.jpg')
        #     _, vis_pred = cap.read()
            
        #     if v['visi']:
        #         current_points = [(int(x), int(y)) for x, y in zip(v['x'], v['y'])]
                
        #         if trajectories:
        #             dist_matrix = calculate_distance_matrix(trajectories, current_points)
        #             row_ind, col_ind = linear_sum_assignment(dist_matrix)
                    
        #             for r, c in zip(row_ind, col_ind):
        #                 if dist_matrix[r, c] < dist_threshold:
        #                     trajectories[r].append(current_points[c])
        #                 else:
        #                     trajectories.append([current_points[c]])
        #                     color = colors(random.randint(0, 99))  # 为新的轨迹选择一个随机颜色
        #                     trajectory_colors.append(tuple(int(255 * c) for c in color[:3]))
        #         else:
        ...
        #     trajectories_show = [traj for traj in trajectories if len(traj) >= min_trajectory_length]  #选择大于阈值的轨迹绘制

        #     for i, traj in enumerate(trajectories_show):
        #         color_pred = trajectory_colors[i]
        #         for j in range(1, len(traj)):
        #             cv2.line(vis_pred, traj[j - 1], traj[j], color_pred, 8)
        #         if cnt < len(traj):
        #             x_pred, y_pred = traj[cnt]
        #             cv2.circle(vis_pred, (int(x_pred), int(y_pred)), 8, color_pred, -1)
            
        #     cv2.imwrite(vis_frame_path, vis_pred)

        # video_path = '{}.mp4'.format(vis_frame_dir)
        # gen_video(video_path, vis_frame_dir, fps=30.0)
        ...
        ###-----------------------------------
            ...
            #     updated_tracks = sort_tracker.update(current_points)

            #     trajectories_show = list(updated_tracks)   # Updated tracks are now directly used as trajectories.

            #     # 动态填充trajectory_colors，确保其大小与trajectories_show相同
            #     while len(trajectory_colors) < len(trajectories_show):
            #         trajectory_colors.append(colors(len(trajectory_colors)))


            #     for i, traj in enumerate(trajectories_show):
            #         color_pred = trajectory_colors[i]
            #         cv2.circle(vis_pred, traj, radius=8, color=color_pred, thickness=-1)
            #####------------------------------------------------------
